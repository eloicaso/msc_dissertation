...
\begin{itemize}
    ...
    If the Log file fills the partition where it is located, Bird will \textbf{automatically} shutdown and \textbf{prevent} any start up attempt until this is resolved. 
\end{itemize}
...
\begin{itemize}
    ...
    \end{itemize}
    \item Load Button: This button will \textbf{select} the file shown in the Text Area and populate the \textit{read-only} Text Box stating which file is actually being edited. Internally, this button will set a \textbf{Lock} variable in the filesystem containing target file name, in order to prevent editing files.
    \item Editing File read-only Text Box: This read-only field is populated with the contents selected in the dropdown menu when the \textit{Load} Button is clicked. Unless this field is populated with a full path to a file, any contents in the TextArea will dropped (\textbf{Submit} button will just refresh the contents of the page).
...
\begin{itemize}
    \item Repository-Contents.md\footnote{Repository-Contents: \href{https://github.com/eloicaso/bgp-bmx6-bird-docn/blob/master/EN/Repository-Contents.md}{Link}.} This file includes the tree structure of Packages repository as well as a description of what is \textbf{each file}.
